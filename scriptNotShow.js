$( document ).ready(async () => {
    ...
    function resultRound(){
        return new Promise(async (resolve, reject) => {
            ...
            let src = null;
            if(result.indexOf("bet-btn--placed") > -1){
                console.log(result);
                console.log(finishRound);
            }
            if(result == "bet-btn bet-btn--placed bet-btn--loss bet-btn--disabled" && finishRound == false){
                src = await client.resultImg();
                console.log(src);
                console.log("LOSE");
                $("#logs").prepend(`
                    <tr>
                        ...
                        <td>Balance: ${parseFloat(parseFloat(client.balance) - parseFloat(client.round[client.numRound])).toFixed(2)}</td>
                        <td><img src="${src}" style="width: 26px;"></td>
                ...
                await timeOut(1000);
                resolve(await startRound());
            }else if(result == "bet-btn bet-btn--placed bet-btn--win bet-btn--disabled" && finishRound == false){
                src = await client.resultImg();
                console.log(src);
                console.log("WIN");
                $("#logs").prepend(`
                    <tr>
                        <td>Round: ${parseInt(client.numRound) + 1}</td> 
                        <td>Bet: ${client.round[parseInt(client.numRound)]}</td>
                        <td>Balance: ${parseFloat(parseFloat(client.balance) + parseFloat(client.round[client.numRound]) * 13).toFixed(2)}</td>
                        <td><img src="${src}" style="width: 26px;"></td>
                ...
                await timeOut(2000);
                resolve(await startRound());
            }else if(result.indexOf("bet-btn--placed") == -1 && status == 1){
